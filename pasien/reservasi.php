...
<body>
        ...
        <div class="modal-dialog modal-dialog-centered modal-dialog-scrollable modal-md">
            <div class="modal-content border-0 shadow-lg" style="border-radius: 24px;">
                ...
                <div class="modal-body p-4">
                    <!-- Search bar for doctor schedule list -->
                    <div class="mb-3">
                        <div class="input-group shadow-sm" style="border-radius: 12px; overflow: hidden;">
                            ...
                            <input type="text" id="searchJadwalInput" class="form-control border-start-0 py-2" placeholder="Cari nama dokter atau poli..." style="border: 1px solid #e0e0e0; outline: none; border-radius: 0 12px 12px 0; font-size: 13px;">
                        </div>
                    </div>
    ...
    <script>
        document.addEventListener('DOMContentLoaded', function() {
            ...
            // Function to filter schedules in the modal based on chosen Poli, Date, and Search Query
            function filterSchedules() {
                const selectedPoli = selectPoli.value;
                const tanggalVal = inputTanggal.value;
                const searchVal = document.getElementById('searchJadwalInput').value.toLowerCase();
                ...
                document.querySelectorAll('.schedule-option-item').forEach(item => {
                    ...
                    const matchesSearch = (searchVal === "" || item.dataset.text.toLowerCase().includes(searchVal));
                    
                    if (matchesPoli && matchesHari && matchesSearch) {
                ...
                let emptyMessage = document.getElementById('no-schedule-message');
                if (visibleCount === 0) {
                    if (!emptyMessage) {
                        emptyMessage = document.createElement('div');
                        emptyMessage.id = 'no-schedule-message';
                        emptyMessage.className = 'text-center py-4 text-muted small';
                        emptyMessage.innerHTML = `<i class="fas fa-search-minus fa-2x mb-2 opacity-50"></i><br>Dokter "${searchVal}" tidak ditemukan`;
                        document.querySelector('#jadwalModal .d-flex.flex-column.gap-2').appendChild(emptyMessage);
                    } else {
                        emptyMessage.innerHTML = `<i class="fas fa-search-minus fa-2x mb-2 opacity-50"></i><br>Dokter "${searchVal}" tidak ditemukan`;
                    }
                } else {
            ...
            document.getElementById('searchJadwalInput').addEventListener('input', function() {
                filterSchedules();
            });
